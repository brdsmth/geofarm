...
| 2 | 2026-09-02 | REVIEW-003 §6 A4 (adopted at milestone M8) | §2.6: the vocabulary membrane gains a location rule — user-facing copy lives only in the surface package, and S10 enforces location as well as vocabulary, with named deviations marked in place. The slice's membrane had held by discipline, not enforcement (REVIEW-003 §2.E). |

**User Experience Implications (Amendment 2).** *Projection:* nothing changes on screen; every word a farmer reads was always meant to come from one place, and now it must. *Concealment:* the lint, the package boundary, and the deviation marker are developer-facing only. *Leak check:* this amendment is the leak check, made mechanical. *Wholeness:* unaffected.
