...
| 8 | **Sources are not people.** What feeds the map is listed as a source under "Where the weather and satellite come from", with "Stop using" and the consequence, never in the sharing list as someone who "sees" the farm. | `sharesHtml()` splits shares by `world.feeds`. Unit test owed on the split (T3 follow-up). |
